...
#pragma warning restore 1692
    ...
    {
        public static LTDescr DOFillAmount(this SlicedFilledImage image, float endValue, float duration)
        {
            if (image == null)
                return null;

            return DOFillAmount(image, endValue, duration, 0f);
        }

        public static LTDescr DOFillAmount(this SlicedFilledImage image, float endValue, float duration, float delay, bool unscaledTime = false, UpdateMethod updateMethod = UpdateMethod.Update, Action onComplete = null)
        {
            if (image == null)
                return null;

            LTDescr tween = LeanTween
                .value(image.gameObject, image.fillAmount, endValue, duration)
                .setDelay(delay)
                .setOnUpdate((float value) => image.fillAmount = value);
